...
contract PresaleManager is ReentrancyGuard{
    ...
    constructor(
        ...
        _owner = msg.sender;
        router = IUniswapV2Router02(0x325E343f1dE602396E256B67eFd1F61C3A6B38Bd);
        feeTo = FeeTo;
    }
    ...
    function addWhiteListAddresses(address[] calldata whiteListUsers) external onlyOwner {
        whiteListEnabled = true;
        for(uint i = 0; i < whiteListUsers.length; i++) {
            users[whiteListUsers[i]].inWhitelist = true;
        }
        emit WhitelistEnabled();
    }

    /** Removes A List Of Addresses From Being WhiteListed */
    function removeWhiteListAddresses(address[] calldata whiteListUsers) external onlyOwner {
        for(uint i = 0; i < whiteListUsers.length; i++) {
            users[whiteListUsers[i]].inWhitelist = false;
        }
    }
    ...
    function isWhiteListed(address holder) external view returns (bool) {
        return users[holder].inWhitelist;
    }
    ...
    receive() external payable {
        ...
        if (whiteListEnabled) {
            require(users[msg.sender].inWhitelist, 'Sender Not Whitelisted');
        }
